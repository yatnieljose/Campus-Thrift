...
class ManageAccountTk(Tk):
    ...
    def __init__(self, account, traceback):
        Tk.__init__(self)
    ...
    def change_password(self):
        ...
        self.lbl_new_password = ttk.Label(self.frame_content, text="New Password").grid(row=7, column=1)
        self.entry_new_password = ttk.Entry(self.frame_content, width=30, font=("Arial", 10), show="*").grid(row=8, column=1)
        self.lbl_confirm_pw = ttk.Label(self.frame_content, text="Confirm Password").grid(row=9, column=1)
        self.entry_confirm_pw = ttk.Entry(self.frame_content, width=30, font=("Arial", 10), show="*").grid(row=10, column=1)
        self.btn_save_pw = ttk.Button(self.frame_content, text="Save", command=self.save_password).grid(row=11, column=0)
    ...
    def save_password(self):
        ...
        if (new_pw == confirm_pw):
            ...
            messagebox.showinfo(title="Password Mismatch", message="Passwords do not match")