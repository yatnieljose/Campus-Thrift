...
class BuyListing(ttk.Frame):
    def __init__(self, master):#, item_name, item_type, min_bid):
        ...
        ttk.Frame.__init__(self, master, width=1400-(2*RIDGE_PAD), height=55, relief=RIDGE)
        """
        self.item_frm = ttk.Frame(master, width=900-(2*RIDGE_PAD), height=55, relief=RIDGE)
        self.item_frm.propagate(0)
    ...
    def fill_labels(self, item_name, item_type, min_bid):
        ...
        ttk.Label(self, text=item_type).place(x=465+RIDGE_PAD, y=RIDGE_PAD)
        ttk.Label(self, text="$" + str(min_bid)).place(x=(465*2)+RIDGE_PAD, y=RIDGE_PAD)